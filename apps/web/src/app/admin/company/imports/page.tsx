...
const IMPORT_TYPES = [
  {
    ...
    description: 'Importa o actualiza productos. Resolución automática de líneas y categorías.',
    icon: '📦',
    columns: 'name, sku, line, category, width, depth, height, unit, price_a..e, currency',
  },
  {
    value: 'prices',
    ...
    icon: '💰',
    columns: 'sku, priceType, currency, basePrice',
  },
  {
    value: 'conditions',
    ...
    icon: '📋',
    columns: 'sku, line, conditionType, description, active',
  },
];
...
// ─── Componente de tipo de importación con badge ─────────────────────────────
...
export default function CompanyImportsPage() {
  ...
  // Polling para actualizar estados dinámicos
  useEffect(() => {
  ...
  const handleDownloadTemplate = () => {
    ...
    const url = `${baseUrl}/imports/template/${selectedType}`;
    ...
    fetch(url, {
      ...
      .then(blob => {
        const filename = `plantilla_${selectedType}_xlayout.csv`;
  ...
  const handleSubmit = async () => {
    if (!selectedFile) {
      setFileError('Selecciona un archivo antes de continuar.');
      return;
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: (job: any) => {
        ...
        const s = job.summary;
        if (s.error) return <span className="text-xs text-red-600 font-medium">{s.error.substring(0, 45)}...</span>;
        return (
          <div className="flex flex-col">
            <span className="text-xs font-semibold text-slate-700">
              {s.succeeded} / {s.total} <span className="text-[10px] font-normal text-slate-400 ml-1">exitosos</span>
            </span>
            {s.failed > 0 && (
              <button
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Importa productos con sus líneas, categorías y precios de forma automatizada.
          </p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 border border-red-100 bg-red-50/30 rounded-lg">
              <h4 className="text-sm font-bold text-red-800 mb-3 flex items-center gap-2">
                ...
                Errores detectados ({errorDetailsJob.summary?.failed}):
              </h4>
              <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2">
                ...
                {(!errorDetailsJob.summary?.errors || errorDetailsJob.summary?.errors.length === 0) && (
                  <p className="text-xs text-slate-500 italic">No hay detalles específicos disponibles.</p>
                )}
              </div>
            </div>
            
            <p className="text-xs text-slate-500 italic">
              * Nota: Solo se muestran los primeros 50 errores para optimizar el rendimiento.
            </p>
          </div>
        )}
      </AdminModal>
      ...
      <AdminModal
        ...
        footer={<>
          ...
          <AdminButton
            ...
            disabled={!selectedFile || !!submitResult?.success}
          >
            {isSubmitting ? 'Enviando...' : 'Iniciar Importación'}
          </AdminButton>
        </>}
      >
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-2">
              {IMPORT_TYPES.map(t => (
                <button
                  ...
                  onClick={() => { setSelectedType(t.value); setSelectedFile(null); setFileError(null); setSubmitResult(null); }}
          ...
          <div className="border-t border-slate-100 pt-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-200 shadow-inner">
              <div className="flex-1">
                <p className="text-sm font-semibold text-slate-700">{activeType.label}</p>
                <p className="text-[10px] text-slate-400 font-mono mt-0.5 break-all">{activeType.columns}</p>
              </div>
              <button
                type="button"
          ...
          <div className="border-t border-slate-100 pt-4">
            ...
            <FileDropZone
              ...
              onClear={() => { setSelectedFile(null); setFileError(null); }}
              error={fileError}
            />
          </div>
        </div>
